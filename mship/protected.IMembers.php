...
function tab_Info(&$MemberID,&$row,$fname,$args)
  {
  $s="";
  if ($row->Subtitle) $s.="<b>'$row->Subtitle'</b><br/>";

  if ($row->StreetAddress) $s.="<b>Address:</b> $row->StreetAddress ";
...
function Browse($args)
  ...
  $this->qpcall=DBQuery("SELECT JSBPageID AS CatalogID,Caption,State FROM jsb_Pages WHERE SysContext='".$cfg['Settings']['mship']['CatalogContext']."' AND State=1","CatalogID");
  ...
  if ($CatalogID) print "<h2>$_[MEMBER_CATALOGSECTION] '".langstr_get($cname)."'</h2>";

  if ($CatalogID)
    {
    print "<table border=0 width='100%' cellpadding='10'><tr valign='top' class='bgdown'><td>";
    
    $_ENV->OpenForm(array(ReloadOnOk=>1,Modal=>1,
		  Action=>ActionURL("mship.IMembers.UpdateCatalog.b"),Align=>"center", Width=>"100%"));
		$_ENV->PutFormField(array(Type=>'hidden',Name=>'CatalogID',Value=>$CatalogID));
		$_ENV->PutFormField(array(
			Name=>"CatalogCaption",Size=>40,
			Caption=>$_['MEMBER_CATALOGSECTION'],
			Type=>"langstring",
			Value=>$cname,
			Required=>1));
		$_ENV->PutFormField(array(Type=>'checkbox',Caption=>$_['MEMBER_CATALOGSECTIONVISIBILITY'],Name=>'Visible',Value=>($this->qpc->Top->State!=2)?1:0));
		$_ENV->CloseForm();
		
    print "</td>";

    print "<td width='50%'><h3>Картинка этого типа участников</h3>";
  ...
  if ($this->qpcall) foreach ($this->qpcall->Rows as $aCatalogID=>$row)
    {
    if ((!$aCatalogID)||($aCatalogID==$CatalogID)) continue;
  ...
  $args=array(
    Action=>ActionURL("mship.IMembers.DoAction.b"),
    Modal=>1,
    Fields=>array(
    ...
    Filters=>array(
      array(Caption=>"Выберите тип видимости участников",
  ...
  if ($PageNo==0)
    {print "<hr><h6>$_[HINT_REGISTERING_MEMBER]</h6>";
    }
  }
