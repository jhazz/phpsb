...
function replaceVars($matches){
	...
	function DisplayBriefcases($args) {
	  ...
	  if ($Closeable) {
			print "<table cellpadding='10' width='100%'><tr><td class='bgdown'>
			  <div id='bcaselist1'><a href='javascript:;' 
			    onClick='document.getElementById(\"bcaselist2\").style.display=\"block\"; 
			    document.getElementById(\"bcaselist1\").style.display=\"none\";'>[+] Показать другие заявки</a></div>
			  <div id='bcaselist2' style='display:none;'><h2>Ваши заявки</h2>";
	  }
	 	$this->BriefcaseTypeID=$BriefcaseTypeID;
	  if (!$UserID) {
		  $wh="SessionKey='$_SESSION->SessionKey'";
		...
		$q=DBQuery("SELECT * FROM doc_Briefcases WHERE $wh ORDER BY DateOpen","BriefcaseID");
		
		
	  $_ENV->PrintTable($q,array(
	    Action=>ActionURL("doc.PBriefcase.ListAction.b"),
	    HiddenFields=>array(BriefcaseTypeID=>$BriefcaseTypeID),
	    Fields=>$fields,
	    Width=>'100%',
	    Fields=>array(
	    	Active=>"Текущий",
	      Caption=>"Название временного пакета документов",
	...
	#      DateClose=>"Дата закрытия",
	    ...
	    Buttons=>array(array(Kind=>'add',FormAction=>'add',Caption=>'Открыть новый заказ')),
	...
	function tab_Current($BriefcaseID,&$data,$f,$a) {
		global $_SESSION;
		if ($_SESSION->doc_Briefcase[$data->BriefcaseTypeID]==$BriefcaseID) print "Текущий";  
		else  {
			if ($data->DateClose) print "Закрыт"; else print "<a href='?bcaseid=$BriefcaseID&setactive=1'>Открыть текущим</a>";
		}
	}
