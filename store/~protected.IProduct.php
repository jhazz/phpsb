...
var $RoleAccess=array(PriceUploader=>"Open,UpdatePage,UpdateControl,EditPage,EditBackendMenu,ExplorerTree");
...
function tab_ProductName(&$ProductID,&$row,$fname,$args)
  ...
  print "<a href='javascript:;' onClick='OpenModal({url:\"".ActionURL("store.IProduct.EditProduct.b",array(EditProductID=>$ProductID))."\",reloadOnOk:1})'>".langstr_get($row->$fname)."</a><br>$t";
  }
function tab_SubName (&$ProductID,&$row,$fname,$args)
  {
  if ($ProductID != $args[EditProductID])
    {
    print "<a href='javascript:;' onClick='OpenModal({url:\"".ActionURL("store.IProduct.EditProduct.b",
...
function EditGroup($args) {
  ...
  $qg=DBQuery ("SELECT Caption,Title,MetaData
    FROM jsb_Pages WHERE JSBPageID=$GroupID AND
    SysContext='".$cfg['Settings']['store']['ProductGroupsContext']."'");
  if (!$qg) return;
  ...
  if ($qprod) {
    ...
    $args=array(
      ...
      Fields=>array(
        ...
        HotNewSale=>"!",
        OrderNo=>$_[ORDERNO]),
      FieldHooks=>array(Name=>tab_ProductName,OrderNo=>tab_OrderNo,HotNewSale=>tab_HotNewSale,BCPrice=>tab_BCPrice),
      ShowCheckers=>1,
      ShowDelete=>1,
      ShowOk=>1,
      TableStyle=>1,
      ...
      ColAligns=>array(OrderNo=>'center',HotNewSale=>'center',BCPrice=>'right'),
      ThisObject=>&$this);
    $_ENV->PrintTable($qprod,$args);
  }

  print "<form method='post' onSubmit='this.target=OpenModal({reloadOnOk:1});' action='".ActionURL("store.IProduct.AddProduct.b",array(GroupID=>$GroupID))."'>
...
function UpdateProduct($args)
  ...
  if (($EditProductID>0)&&($ProductName))
    ...
    if ($cfg['Settings']['store']['AskPackageWS'])
      ...
      $Height*=$mss[$msize];
      $Length*=$mss[$msize];
      $PackageWS=",PackageWS='".$Weight.'x'.$Width.'x'.$Height.'x'.$Length."' ";
...
function EditProduct ($args)
  ...
  switch($PageTab) {
    case 'main':
      ...
      print "</td><td>";
      $_ENV->OpenForm(array(Name=>"Form1",ShowCancel=>1,Action=>ActionURL("store.IProduct.UpdateProduct.b"),Align=>"left"));
      ...
      $PackageWS=explode ("x",$qp->Top->PackageWS);
...
#      print "<br/><textarea class='inputarea' cols='45' rows='5' style='width:100%' name='Teaser'>".$qp->Top->Teaser."</textarea>
#      <br/>";
...
#      print "</tr></form></table><br>";
      ...
      case 'prices':
        ...
        print "<br/><form onSu bmit='this.target=OpenModal({reloadOnOk:1});'
        ...
        foreach ($qcols->Rows as $ColumnID=>$Column)
          ...
          print "<tr ><td align='right' class='small'><b>$Column->Caption:</b></td><td>$Currency->Prefix</td><td align='right'>";
...
#            $PriceValueStr=number_format(floatval($PriceValue),$Currency->Decimals,$__[DECIMAL_SEPARATOR],$__[THOUSAND_SEPARATOR]);
            print "<input onChange='ColChanged()' onKeyUp='ColChanged()' style='text-align:right' type='text' class='inputarea' size='14' maxlength='14' id='Price_$ColumnID' name='Prices[$ColumnID]' value='$PriceValueStr'/>";
      ...
      ?><script>
    function hideForm2(){GetObj("div_form2").style.visibility='hidden';}
    function hideAdd() {hideAddLink();div=GetObj("newoptionsdiv");if (div) div.style.visibility='hidden';}
    function hideAddLink() {var div=GetObj("AddLink"); if (div) {div.style.display='none'}}
    function blurNewOptions(){var box=GetObj("new_options[]");if (box){for (i=0;i<box.options.length;i++) {box.options[i].selected=false;}}}
    function hideForm1(){GetObj("div_form1").style.visibility='hidden';}
    function showOptionsForm() {GetObj("newoptionsdiv").style.display="block";hideAddLink();}
    </script><?
      ...
      if ($quo || $new_options)
        ...
        if ($new_options) {if ($uo_list) {$uo_list.=",";} $uo_list.=implode(",",$new_options);}
        ...
        print "<form onSubmit='this.target=OpenModal();' method='post' action='".ActionURL("store.IProduct.UpdateProductOptions.b")."'>
        <input type='hidden' name='PageTab' value='$PageTab'>
        <input type='hidden' name='EditProductID' value='$EditProductID'>
        <table border=0>";
        foreach ($qo->Rows as $OptionID=>$row)
          ...
          print "<tr><input type='hidden' name='OptionIDs[]' value='$OptionID'/><td class='co'>"
            .langstr_get($row->Name)."</td><td class='ce'>"
            .langstr_get($row->Unit)."</td><td class='ce'>";
          $_ENV->PutDropDown(array(Size=>15,Editable=>1,Name=>"opv_$OptionID",Value=>langstr_get($v),ValueSetName=>"op_$OptionID"));
          print "</td><td class='ce'>".langstr_get($row->Info)."</td></tr>";
          }
        print "<tr><td colspan='4' align='center'><br>";
        $_ENV->PutButton('submit');
        print "</td></td></tr></table></form>";
        }
      print "</td></tr></table>";
      # all options for 'new option' drop down
      ...
      if ($qo) {
        ...
        foreach ($qo->Rows as $OptionID=>$row)
          ...
          $s.="<option value='$OptionID'>".langstr_get($row->Name)."</option>";
          }
        if ($s) {
          $s="<br><select style='width:100%' onClick='hideForm2()' class='inputarea' size='8' id='new_options[]' name='new_options[]' multiple>$s</select><br>";
          $s.=$_ENV->PutButton(array(ToString=>1,Action=>'submit',Kind=>'add'));
          }
        }

      $s="<div id='newforms'>
      <table width='100%' cellpadding='5'><tr valign='top'><td class='bgdown'><div id='div_form1'>
        <h2>$_[SELECT_ONE_OR_MORE_OPTIONS_TO_ADD]</h2>
        <form name='addoptform' id='addoptform' method='get' action='"
        .ActionURL("store.IProduct.EditProduct.b")."'>$s
        <input type='hidden' name='EditProductID' value='$EditProductID'>
        <input type='hidden' name='PageTab' value='$PageTab'>
        </form>
      </div></td><td class='bgdown'>";

      $s.="<div id='div_form2'>
       <h2>$_[ADD_NEW_OPTION]</h2>
       <form name='newoptform' id='newoptform' onSubmit='this.target=OpenModal({reloadOnOk:1});'
       method='post' action='".ActionURL("store.IProduct.AddNewOption.b")."'>
       <input type='hidden' name='PageTab' value='$PageTab'>
       <input type='hidden' name='EditProductID' value='$EditProductID'>
      <br><table>
       <tr><td>$_[OPTION_NAME]:</td><td><input onFocus='hideForm1()' class='inputarea' type='text' maxlength='100' name='newoptionname' value=''></td></tr>
       <tr><td>$_[OPTION_VALUE]:</td><td><input class='inputarea' type='text' maxlength='100' name='newoptionvalue' value=''></td></tr>
       <tr><td>$_[OPTION_UNIT]:</td><td><input class='inputarea' type='text' maxlength='20' size='10' name='newoptionunit' value=''></td></tr>
       <tr><td>$_[OPTION_INFO]:</td><td><input class='inputarea' type='text' maxlength='100' name='newoptioninfo' value=''></td></tr>
       <tr><td></td><td>".$_ENV->PutButton(array(ToString=>1,Action=>'submit',Kind=>'add'))."</td></tr>
       </table>
       <br><br></form>
       </div></td></tr></table></div>";
      ...
      if ($quo)
        {
        print "<br><div id='AddLink'><a href='javascript:;' onclick='showOptionsForm();'>$_[ADD_ONE_OR_MORE_OPTION]</a></div>
        <div id='newoptionsdiv' style='display:none;'>$s</div></div>";
        }
      else {
        print "<div id='newoptionsdiv'><h2>$_[ADD_ONE_OR_MORE_OPTION]</h2>$s</div>";
        }
      break;
...
WHERE p.BaseProductID=$BaseProductID",array("ProductID","OptionID"));
      ...
      print $_[HINT_ADD_SUBPRODUCT];
      print "<table cellpadding='5'><form method='post' onSubmit='this.target=OpenModal();' action='"
      .ActionURL("store.IProduct.AddSubProduct.bm",array(BaseProductID=>$BaseProductID))."'>
    <tr>
      <td bgcolor='#d8d8d8'>$_[PRODUCT_SKU]:<br>
      <i>$BaseSKU</i><br>
      ...
      <input type='text' name='NewProductName' class='inputarea' size='80' maxlength='250' value='+'><br>
      </td>
    </tr>
    <tr>
      <td></td><td><input type='submit' value='$__[CAPTION_ADD]' class='button'></td>
    </tr>
    </form></table>";
  ...
  print "</td></tr><tr><td colspan='20' align='right'>";
...
function UpdateProductOptions($args)
  ...
  return array(ForwardTo=>ActionURL("store.IProduct.EditProduct.b",array(EditProductID=>$EditProductID,PageTab=>'options')));
  }
