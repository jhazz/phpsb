...
initModule:function(){
	...
	P$.$.registerClass({
		...
		create:function(owner,c,ca){
			...
			if(typeof owner=='string')owner=P$.$.allcontrols[owner];
			_.controls={},
			_.subscribers={},
			_.le={},
			_.owner=owner;
		...
		owner.controls[_.hw]=W.allcontrols[_.hw]=_;
	},
	...
	free:function(e){
		...
		for(i in _.controls)_.controls[i].free();
		if(_.owner)delete(_.owner.controls[_.hw]);
		if(_.onfree)_.onfree();
		delete W.allcontrols[_.hw];
...
subscribe:function(a,w,p){
	w=(!w)?p.Wn:(typeof w=='string')?(W.allwins[w]||W.allcontrols[w]):w;
...
dispatchEvent:function(e,w){ 
	...
	if(d){
		if(d&1)e.dir&=1,W.dispatchEvent(e,w.parent);// bubbling - избегаю циклов если вдруг захотели во все стороны пустить событие
			...
			e.dir=d=d&2;
			for(i in w.controls)W.dispatchEvent(e,w.controls[i]);
		}
	}
	if(w[et]) w[et](e);
