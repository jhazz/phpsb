P$.$.regModule('sys.templates',{
onUsing:function(params,ldr,p){
	P$.trace('Вызван sys.templates.onUsing()','Источник: '+ldr.name);
	
	},
	// вызывается после того как все модули, от которых зависит данный модуль, загружены
onSkinLoad:function(){
	P$.trace('Вызван sys.templates.onSkinLoad()');
	
},
initModule:function(){
	P$.trace('Вызван sys.templates.initModule()');
	P$.use('sys.templates','sys.templates.skin1','sys.templates.onSkinLoad');
