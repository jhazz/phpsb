...
init:function(){
	var s="",w=400,h=200,wid;
	$.on('resize',this.bodyOnResize,window);
	s+="<div id='focuscatcher' style='position:relative'></div><div id='Pop_container' style='position:absolute; visibility:hidden;left:0; top:0;z-index:10001'>"
	+"<table border='0' cellspacing=0 cellpadding=0><tr valign='top'><td><input type='text' style='width:1; height:1; border:none;' id='Pop_focus' onBlur='W.disp.prepareClosePopup();'></td>"
	+"<td class='pop_frame' id='Pop_inner'></td></tr></table></div>";
	$.w(s);
	W.disp.initEvents(W);
},
initEvents:function(w){
	if(ie){
		$.on('mouseup',  this.mouseUp,w.doc);
		$.on('mousedown',this.mouseDown,w.doc);
		$.on('mousemove',this.drag,w.doc);
	}
},
mouseDown:function(e){
	var p,i,topW=0,topZ=0,obj,m,d=W.disp,hwnd;
	obj=(ie)?e.srcElement:e.target;
	m=obj.attributes.dragMode;
	if (!m) {return;}
	d.dragMode=m=m.value;
	hwnd=obj.attributes.hwnd.value;
	d.dragWnd=d.allwins[hwnd];
	d.toFront(hwnd);
	switch(m){
...
mouseUp:function(){
	...
	if (this.dragWid){
		var ff=$("syswff"+this.dragWid);
		ff.style.visibility='hidden';
	}
	
},
/*startDrag:function(mode,wid,e){
	for(var i in this.stack){if(this.stack[i].wid==wid)this.dragWnd=this.stack[i];}
	if (!this.dragWnd) return;
	this.dragMode=mode;
	this.dragWid=wid;
	switch(mode){
		case "move": this.dragSX=e.screenX-this.dragWnd.x; this.dragSY=e.screenY-this.dragWnd.y; break;
		case "sz_wh": this.dragSX=e.screenX-this.dragWnd.w; this.dragSY=e.screenY-this.dragWnd.h; break;
	}
	e.returnValue=false;
	return false;
},*/
drag:function(e){
	var d=W.disp,m=d.dragMode,p=d.dragWnd;
	if (!m) return;
	var dx=e.screenX-d.dragSX, dy=e.screenY-d.dragSY;
	switch(d.dragMode) {
...
openWin:function(aW,p){
  ...
  //   глобальная W указывает на самое первое окно, относительно которого был создан W.disp
	window.status=aW.testid + ' W.testid='+W.testid;
	var url,st,stackIn,d,w=Int(p.w),h=Int(p.h),fr,i,z,mZ;
	if (!w)p.w=w=400;
	if (!h)p.h=h=150;
	if(!p.title)p.title=aW.defaultTitle;
	p.HWND="WND:"+(this.nextHWND++);
	p.stackIn=stackIn=(p.inside)?aW : this.bottomW;
	d=stackIn.doc.createElement("DIV");
	stackIn.doc.getElementById("Windows").appendChild(d);
	if(p.x!=undefined)p.x=Int(p.x),p.y=Int(p.y),p.autopos=0;else p.autopos=1;
	d.innerHTML=this.getWindowHtml(p);
	this.allwins[p.HWND]=p;
	if(p.isTop){st=stackIn.topwins;} else {st=stackIn.wins;}
	mZ=(p.isTop)?10000:1;
	...
	function dd(i){return stackIn.doc.getElementById(p.HWND+':'+i);}
	p.e={w:dd('w'),a:dd('a'),i:dd('f'),t:dd('t'),tit:dd('tit')};
	p.e.tit.innerHTML='z='+p.zIndex;
	
	$.let(p.e.w.style,{zIndex:p.zIndex,visibility:'visible'});
	...
	if (url){
		...
		p.params.rand=Math.random();
		if(p.isModal)p.params.call='modal';
		for (var k in p.params){
		if (!k.match(this.reservedParams)){url+=s+k+"="+p.params[k]; s="&";}}
		fr.open();
		fr.write(W.strWait);
		fr.close();
		p.e.i.src=url;
