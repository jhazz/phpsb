P$.$.regModule('sys.XML',{
onusing:function(params,ldr,p){P$.trace('Вызван sys.XML.onUsing()','Источник: '+ldr.name);},
initModule:function(){P$.trace('Вызван sys.XML.initModule()');},


// hw - instance hw 
// m - XML node or Object
Instance:function(hw,m){
	var _=this,aa,l,i,a,W=P$.$.sys.win,X=P$.$.sys.XML;
	if((aa=m.attributes)!=null){
		l=aa.length;
		for(i=0;i<l;i++){a=aa.item(i);this[a.nodeName]=a.nodeValue;}
		_.modelNode=m;
	}else{for(i in m)_[i]=m[i];

	_.dom=_.createDOM();
	c=new W.Control(_,owner,htmlNode);
	
	ЛАЖА
	if(_.src){
		_.loader=PFD.createDOMLoader();
		if(_.getAttribute('loadOnStart')){_.loadData();}
	} else {
		_.dom.appendChild(m.cloneNode(true));
	}
return this;
},
put:function(a,owner,htmlNode){
	
	var P=P$.$,c=P.createObject('XML',a,owner,htmlNode);
	
},
# переделать на xx/yy/zz:/aa/bb/cc
# поддержка ../../zz:/aa/bb
# xx,yy - wins[]
# zz - component hw
...
createDOM:function(){var r;try{r=new ActiveXObject("Microsoft.XMLDOM");}catch(e){try{r=document.implementation.createDocument("","",null);}catch(e){alert('XML is not supported by your browser.');}}return r;},
...
extractPathElements:function(p){if(this.reInstance.test(p)){return this.reInstance.exec(p);}else{P$.trace("XML.extractPathElements:Путь не содержит идентификатора instance:",p);}},
findInstanceNode:function(path,multiple){
		var ins,d,r=this.extractPathElements(path);
		if(r) {
			ins=P$.sys.win.allcomponents[r[1]];
			if(!ins){P$.trace('Unknown instance'+r[1],path);return false;}
			d=ins.dom;
			if(!d){return false;}
...
using:function(params,ldr,p){P$.trace('Вызван sys.XML.onusing()','Источник: '+ldr.name);},
initModule:function(){P$.trace('Вызван sys.XML.initModule()');
	P$.$.registerClass({ 
		className:'XML'
	});
