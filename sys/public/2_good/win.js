...
ceTemplate:function(tmpl,a,p,d){
	...
	for(i in t.e)e=t.e[i],a.e[e]=P$(a.id+":"+e,d);
	a.tmplName=tmpl;
	a.tmpl=t;
...
ce:function(i,p,d,ar,id){
	if(!ar)ar={};
	if(!d)d=document;
	var j,t,a,k,s,v,z,e,r=function(g){g=g+'';return g.replace(P$.REargs,function(j,i){with(ar){return eval('('+i+')');}})};
	
	for(t in i)break;a=i[t];
	e=d.createElement(t);
	if(!p)p=d.body; else if(typeof p=="string")p=P$(p,d);
	for(k in a){
		s=a[k];
...
initWin:function(args,d){
	var wf,e,w,h,W=P$.$.sys.win;
	P$.on('mouseup',  W.mouseUp,  d.d);
	P$.on('mousedown',W.mouseDown,d.d);
	P$.on('mousemove',W.mouseMove,d.d);
	
	w=d.Wn={wins:{},topwins:{},testid:Math.round(Math.random()*200)};
	try{
		h=d.w.frameElement.attributes.hwnd.nodeValue; 
		W.ce({div:{id:'W',hwnd:h,P$:h}},0,d.d);
		wf=W.allwins[h];
		if(wf)P$.let(wf,w);else W.allwins[h]=wf=w;
		d.Wn=wf
		//alert('Source '+h+'\n'+P$.dump(P$.D.W.allwins[h]));
	}catch(e){}
	
	P$('Windows',d.d).innerHTML="P$.testid="+w.testid;
	if(!W.inited){
		W.inited=1;
	  P$.on('resize',this.bodyOnResize,window.top);
...
mouseDown:function(e,v){
	...
	if(!h){h=P$('W',obj.ownerDocument);if(h)h=h.attributes.hwnd}
	if(h)h=h.value;
...
mouseUp:function(){
	//var ff,hw=P$W.dragWnd.HWND;
	P$.$.sys.win.dragMode=0;
	//P$.off('mousemove')
//	if (h){ff=P$("syswff"+h);ff.style.visibility='hidden';}
},
...
toFront:function(hw){
	var p,i,j,z,tj,tz=0,w=P$.$.sys.win.allwins[hw],s;
	if(!w)return;
	s=w.stackIn.Wn;
...
openWin:function(a,p){
  ...
  var s,hw,u,st,stackIn,dv,df,w=Int(a.w),h=Int(a.h),fr,i,z,mZ,W=P$.$.sys.win;
	...
	a.e={};
	if(a.url){a.text="<iframe hwnd='"+hw+"' src='' frameborder='0' name='"+hw+"_f' id='"+hw+":iframe' width='"+a.w+"' height='"+a.h+"'>Your browser not support IFRAMEs</iframe>";}
	dv=W.ceTemplate('standardWindow',a,"Windows",a.stackIn.d,a);
	a.e.title.innerHTML='z='+a.zIndex;
	a.e.w.onselectstart=a.e.w.ondragstart=P$.no;
	P$.let(a.e.w.style,{zIndex:a.zIndex,visibility:'visible'});
...
modalResult:function(mr,d){
	var w;
	if(mr.event){
		var a=P$.eventObj(mr.event).attributes.id.value.split('|'),h=a[0];w=P$.$.sys.win.allwins[h];
	}else w=d.Wn;
	P$.run('sys.win.modalResult',{});
...
/*	if (this.modalWait){
	...
	var m=this.stack[this.active];
	if (m){
		P$("syswm"+m.wid).style.visibility='hidden';
		P$("syswfl"+m.wid).style.visibility='hidden';
	}
...
_modalResult:function() {
	...
	if (this.active!=-1){
		P$("syswm"+aw.wid).style.visibility='visible';
		P$("syswfl"+aw.wid).style.visibility='visible';
	}

	if (this.active>0){
		var prevwid=this.stack[this.active-1].wid;
		var o=P$("syswfl"+prevwid);
